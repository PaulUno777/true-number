...
import { Users, GamepadIcon, TrendingUp, TrendingDown, DollarSign, Plus, Trash2, Edit, Eye, UserCheck, UserX, Crown, User, Shield, Home, Target, Trophy, Calendar } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [viewMode, setViewMode] = useState<'overview' | 'users' | 'stats'>('overview');
  ...
  const updateUserMutation = useMutation({
    mutationFn: ({ userId, data }: { userId: string; data: Partial<EditUserForm> }) => 
  ...
  const toggleUserRole = (userData: any) => {
        ...
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <Card className="game-card">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-primary">{globalStats?.totalUsers || 0}</div>
            ...
            <Card className="game-card">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-secondary">{globalStats?.totalGames || 0}</div>
            ...
            <Card className="game-card">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-white">{t('globalWinRate')}</CardTitle>
                <Target className="h-4 w-4 text-accent" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-accent">{globalStats?.globalWinRate.toFixed(1)}%</div>
            ...
            <Card className="game-card">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-white">{t('averageBalance')}</CardTitle>
                <DollarSign className="h-4 w-4 text-green-400" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-green-400">${globalStats?.averageBalance.toFixed(2) || 0}</div>
                <p className="text-xs text-muted-foreground mt-1">{t('averageBalance')}</p>
              </CardContent>
            </Card>
          </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs opacity-70">{t('manageUsersDescription')}</div>
                  </div>
                </Button>
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs opacity-70">{t('playGameDescription')}</div>
                  </div>
                </Button>
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs opacity-70">{t('viewStatsDescription')}</div>
                  </div>
                </Button>
              </div>
            </CardContent>
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className={`flex items-center justify-center w-8 h-8 rounded-full text-white font-bold text-sm ${
                        index === 0 ? 'bg-yellow-500' : index === 1 ? 'bg-gray-400' : index === 2 ? 'bg-amber-600' : 'bg-gradient-to-r from-accent to-secondary'
                      }`}>
                        {index + 1}
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      <div className="font-bold text-accent">${player.balance}</div>
                    </div>
                  </div>
                )) || (
          ...
          <CardContent>
            ...
            {showCreateForm && (
              <Card className="mb-6 border border-accent/50 bg-white/5">
                ...
                <CardContent>
                  <form onSubmit={createUserForm.handleSubmit(onCreateUser)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input
                    ...
                    >
                      ...
                      <option value="ADMIN" className="bg-gray-800">{t('admin')}</option>
                    </select>
              ...
              <Card className="mb-6 border border-blue-500/50 bg-blue-500/5">
                <CardHeader>
                  <CardTitle className="text-lg text-blue-400 flex items-center space-x-2">
                    ...
                    <span>{t('editUser')}: {editingUser.username}</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <form onSubmit={editUserForm.handleSubmit(onEditUser)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input
            ...
            <div className="space-y-4">
              {users?.data?.map((userData: any, index: number) => (
                <div
                  key={userData.id}
                  className={`group p-6 rounded-xl glass-effect border border-white/20 hover:border-accent/50 transition-all duration-300 hover:scale-[1.02] ${
                    selectedUser?.id === userData.id ? 'ring-2 ring-accent/30 border-accent/50' : ''
                  }`}
                  style={{
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div className="relative">
                        <div className={`w-14 h-14 rounded-full flex items-center justify-center text-white font-bold transition-transform duration-300 group-hover:scale-110 ${
                          userData.role === 'ADMIN' 
                            ? 'bg-gradient-to-r from-red-500 to-pink-600 shadow-red-500/30' 
                            : 'bg-gradient-to-r from-blue-500 to-cyan-600 shadow-blue-500/30'
                        } shadow-lg`}>
                          {userData.role === 'ADMIN' ? <Crown className="h-6 w-6" /> : <User className="h-6 w-6" />}
                        </div>
                        <div className={`absolute inset-0 rounded-full ${
                          userData.role === 'ADMIN' ? 'bg-red-500' : 'bg-blue-500'
                        } opacity-20 blur-xl group-hover:opacity-40 transition-opacity duration-300`}></div>
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center space-x-2">
                          <div className="font-semibold text-white text-lg">{userData.username}</div>
                        ...
                        <div className="text-xs text-muted-foreground">{userData.phone}</div>
                      </div>
                    </div>
                    
                    <div className="flex items-center space-x-6">
                      <div className="text-right">
                        <div className="text-xl font-bold text-accent">${userData.balance}</div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          ...
                          title={userData.role === 'ADMIN' ? t('makeClient') : t('makeAdmin')}
                          disabled={updateUserMutation.isPending}
                        >
                          {updateUserMutation.isPending && updateUserMutation.variables?.userId === userData.id ? (
                        ...
                        {userData.id !== user?.id && (
                          <Button
                            ...
                            variant="outline"
                            onClick={() => {
                              if (window.confirm(t('confirmDelete', { username: userData.username }))) {
                    ...
                    <div className="mt-6 pt-6 border-t border-white/20">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="space-y-2">
                          <h4 className="font-semibold text-white">{t('accountInfo')}</h4>
                          <div className="space-y-1 text-sm text-muted-foreground">
                            <div>ID: {userData.id}</div>
                            <div>{t('created')}: {new Date(userData.createdAt).toLocaleDateString()}</div>
                            <div>{t('updated')}: {new Date(userData.updatedAt).toLocaleDateString()}</div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-1 text-sm text-muted-foreground">
                            <div>{t('email')}: {userData.email}</div>
                            <div>{t('phone')}: {userData.phone}</div>
                          </div>
                        </div>
                        
                        <div className="space-y-2">
                          <h4 className="font-semibold text-white">{t('stats')}</h4>
                          <div className="space-y-1 text-sm text-muted-foreground">
                            <div>{t('balance')}: ${userData.balance}</div>
                            <div>{t('role')}: {userData.role}</div>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="text-center py-12 space-y-4">
                  ...
                  <p className="text-xl text-muted-foreground">{t('noUsers')}</p>
                </div>
              )}
            </div>
          </CardContent>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <Card className="bg-green-500/5 border-green-500/30">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-green-400">{t('wonGames')}</p>
                        <p className="text-3xl font-bold text-green-400">{globalStats?.wonGames || 0}</p>
                      </div>
                ...
                <Card className="bg-red-500/5 border-red-500/30">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-red-400">{t('lostGames')}</p>
                        <p className="text-3xl font-bold text-red-400">{globalStats?.lostGames || 0}</p>
                      </div>
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className={`flex items-center justify-center w-10 h-10 rounded-full text-white font-bold ${
                        index === 0 ? 'bg-yellow-500 shadow-yellow-500/30' : 
                        index === 1 ? 'bg-gray-400 shadow-gray-400/30' : 
                        index === 2 ? 'bg-amber-600 shadow-amber-600/30' : 
                    ...
                    <div className="text-right">
                      <div className="font-bold text-accent text-xl">${player.balance}</div>
                      {index < 3 && (
                        <div className="text-xs text-yellow-400">🏆 {t('medal')}</div>
                      )}
                    </div>
                  </div>
                )) || (
